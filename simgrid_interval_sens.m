function [ps,t_out,X,Y,blockout] = simgrid_interval_sens(ps,t,t_next,x0,y0,opt,blockout)
...
if ~all(is_first_subgraph)
    ...
    [ps1,t_out1,X1,Y1,blockout] = simgrid_interval_sens(ps1,t,t_next,x01,y01,opt,blockout);
    [ps2,t_out2,X2,Y2,blockout] = simgrid_interval_sens(ps2,t,t_next,x02,y02,opt,blockout);
...
%         ps.shunt(:,C.sh.current_Q) = imag(S_zipe(ps.shunt(:,C.sh.type)==1))* ps.baseMVA ;
        ...
        if temp_ref, ps.bus(ismember(ps.bus(:,1),ps.gen(ref,1)),C.bu.type) = temp_ref; end
            ...
            if ~isempty(relay_event)
                ...
                [ps,t_out_down,X_down,Y_down,blockout] = simgrid_interval_sens(ps,t_down,t_next,x_end,y_end,opt,blockout);
