...
switch mode
    case 'temperature'
        ...
        % NRO added by falah to make normal opreation tempreature 55
%         NRO=55-(relay(:,C.re.temp_R).*ps.branch(:,C.br.Imag_f).^2./relay(:,C.re.temp_K));
...
%         overload_max    = Imax * 1.5 * 5;    % 5 seconds at a 50% overload
        ...
        % Falah Modification
        Imax            = ps.branch(:,C.br.rateA)/ps.baseMVA;
        overload_max    = Imax *  1.8;    % 5 seconds at a 50% overload
