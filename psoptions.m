...
opt.sim.max_iters = 30;         % default number of newton iterations for trapezoidal solver
opt.sim.tolerance = 1e-9;       % newton convergence tolerance
opt.sim.draw = true;
% opt.sim.overload_time_limit = 10*60; % number of seconds that the branch can sit at its rateC level (PSS/E manual)
opt.sim.overload_time_limit = 0;
opt.sim.t_eps = 1e-16;
...
opt.sim.uvls_limit = 0.87;     % threshold at which under voltage load shedding occurs
opt.sim.uvls_delta = 0.25;     % the fraction of load that is shed during simulation if under voltage
opt.sim.ufls_limit = 0.975;     % threshold at which under frequency load shedding occurs
